...
<head>
    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/administrator/assets/css/match_operate.css">
...
<script type="text/javascript">
        ...
        $.post(url,function (data) {
            ...
            for (var i = 0;i<data.length;i++) {
                html = html +"<tr>\n" +
                    ...
                    "            <td>"+data[i].match_now+"</td>\n" +
                    "            <td>"+data[i].match_max+"</td>\n" +
                    "            <td><button type=\"button\" class=\"btn btn-danger btn-delet\" onclick='delete_match(this)'>取消比赛</button><input type='hidden' value='"+data[i].match_id+"' /></td>\n" +
                    "        </tr>";
            }
            $("#match_table").html(html);
    ...
    function delete_match(obj) {
        var message = confirm("确定要取消比赛吗?");
        if(message == true){
            var match_id = $(obj).next().val();
            var url = "${pageContext.request.contextPath}/match_delete";
            var param = {
                "match_id":match_id,
            }
        ...
        $(location).attr('href', 'match_operate.jsp');
    }
</script>
</body>
