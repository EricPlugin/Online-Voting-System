...
<head>
    ...
    <!-- 自定义样式 -->
    <link rel="stylesheet" href="${pageContext.request.contextPath}/confrontation/assets/css/confrontation_operate.css">
    <!-- 自定义样式 -->
...
<div class="page_body">
    <table class="table-hover">
        ...
        <tbody id="confrontation_table"></tbody>
    </table>
</div>
...
<script type="text/javascript">
    $(function () {
        var html = "";
        var url = "${pageContext.request.contextPath}/confrontationCnd_getAll"
        $.ajax({
            url: url,
            ...
            success: function (data) {
                for(var i = 0;i<data.length;i++){
                    html = html+"<tr>\n" +
                        "            <td>"+data[i].match_name+"</td>\n" +
                        "            <td>"+data[i].player_1_name+"</td>\n" +
                        "            <td>"+data[i].player_2_name+"</td>\n" +
                        "            <td><button class=\"btn btn-primary btn_player_table\" onclick='confrontation_release(this)'>发布对战</button><input type='hidden' value='"+data[i].confrontation_id+"'></td>\n"+
                        "        </tr>";
                }
                $("#confrontation_table").html(html);
            }
        });
        ...
        $("td.user_table_td").on("click","button.btn.btn-primary.btn-alter",function () {
            alert(111);
        });
    });
    function confrontation_release(obj) {
        var confrontation_id = parseInt($(obj).next().val());
        var url = "${pageContext.request.contextPath}/confrontation_release";
        var param = {
